...
class Player:
    ...
    def setChoices(self, choices, input_dict):
        ...
        if any((choice not in input_dict) for choice in choices):
            raise exception.PlayerInputError("Player choice is unknown. Try again.")
        # Make sure no more than three choices are made
        elif len(choices) > 3:
            raise exception.PlayerInputError("Too many choices. Try again.")
        # Make sure no names are repeated
        elif len(choices) != len(set(choices)):
            raise exception.PlayerInputError("Player name repeated. Try again.")
        # Make sure the player didn't chose themself
        elif any((choice == self.name) for choice in choices):
            raise exception.PlayerInputError("Can't choose yourself!")
        else:
    ...
    def setNumCities(self, num_cities):
        if isinstance(num_cities, str):
            if not num_cities.isdigit():
                raise exception.PlayerInputError("Integer not entered for number of cities. Try again")
            else:
        ...
        if num_cities < 0:
            raise exception.PlayerInputError("Negative integer entered for number of cities. Try again")

        self.num_cities = num_cities

    # Get a list of choices for this player that overlap with a given list
    def getChoicesThatOverlapWithList(self, list):
        return helper.getOverlapBetweenLists(self.choices, list)
